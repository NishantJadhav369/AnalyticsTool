...
for level in range(1, 6):    
    ...
    if value and group_level: # Enter if value and group_level is selected by user
        ...
        if type(regression_output)==str:
            ...
            st.text("Very few claims available for Regression using the mentioned columns")
            st.markdown("### Detailed Data View")
            detailed_view = eda_slice.loc[:, eda_slice.columns != 'UID']
...
#                 break
            ...
            if (level+1) ==2:
                options = level2
                default_level = ['Minor Category','ORIGINAL_MFG_PLANT','BSR','REASON','Claim Component','BRAND','SALES_REGION_ID','GL_PERIOD_MONTH']

            options = [ option for option in options if option not in table['Dimension'].values ]
            default_level = [ col for col in default_level if col not in table['Dimension'].values ]
            if value and level>1 and (group_level=='Claim Component'):
                # print("PM ",group_level,value)
                temp_slice = eda_slice[eda_slice[group_level]==value].copy()
                cost_result,eda_slice = eda_table(temp_slice,text_component,ship, filter_dim,filter_val,'PIECEMARK',level) # eda call
                top_results = cost_result[:10]
                top_results['PIECEMARK'] = top_results['PIECEMARK'].astype(str)
